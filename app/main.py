...
from app.config import ALLOWED_ORIGINS, DEBUG, SERVICE_NAME
...
def _xml_stream(xml_bytes: bytes, filename: str) -> StreamingResponse:
    headers = {
...
# ---------- MVP endpoints tailored for Webflow ----------

@app.post("/v1/create-from-description-xml", tags=["mvp"])
async def create_from_description_xml(
    description: str = Form(..., description="Free-text description of the desired form"),
    ...
    if not v["ok"]:
        # Return issue list in headers and body for quick debugging
        raise HTTPException(status_code=400, detail={"issues": v["issues"]})

    xml_bytes = compose_xml(cir)
    filename = f"{cir['meta'].get('ref','form')}.xml"
    return _xml_stream(xml_bytes, filename)
...
async def compose_xml_endpoint(cir: dict):
    ...
    xml_bytes = compose_xml(cir)
    return PlainTextResponse(xml_bytes.decode("utf-8"), media_type="application/xml")
